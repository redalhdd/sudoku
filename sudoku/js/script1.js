...
function selectNumber(button) {
  if (selectedInput) {
    ...
    selectedInput = null; // Réinitialiser après l'écriture

  } else {
    showNotification("Veuillez d'abord sélectionner une case.");
  }
}
...
function solveSudoku(changeUI) {
  ...
  for (var i = 0; i < puzzle.length; i++) {
    for (var j = 0; j < puzzle[i].length; j++) {
      ...
      mistakes = mistakes + (result > 2 ? 1 : 0);
      addClassToCell(
...
function displayMistakes(){
  ...
  inputMistake.innerText = "Erreur(s) : "+ mistakes + "/3";
}
